...
struct dict_t* dictionary_new(char *data_file, size_t num_items) {
  ...
  dict->num_items = malloc(sizeof(num_items));
  // dict->base = mmap()
  dict->path* = "/usr/share/dict/words";
  dict->base* = mmap(NULL, dict.num_items, PROT_READ | PROT_WRITE, MAP_SHARED, dict.path, )
...
int dictionary_open_map(struct dict_t *dict) {
  ...
  ftruncate(fd, ditionary_len(dict))
  return mmap(NULL, dictionary(len(dict)), PROT_READ | PROT_WRITE, MAP_SHARED, fd,0)
